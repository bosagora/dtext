...
public alias void delegate(in char[]) @safe FormatterSink;
...
private alias void delegate(in char[], ref const(FormatInfo)) @safe ElemSink;
    ...
    @trusted
{
    ...
    scope ElemSink elemSink = (in char[] str, ref const(FormatInfo) f)
    {
        widthSink(sink, str, f);
    };

    // Main loop
    scope const(char)[] fmt_ = fmt;
    while (fmt_.length)
    {
        info = consume(sink, fmt_);
...
private void widthSink (scope FormatterSink sink, in char[] str, ref const(FormatInfo) f)
    @trusted
{
    if (f.flags & Flags.Width)
    {
        ...
        if (f.flags & Flags.Crop)
        {
            ...
            {
                assert((f.flags & Flags.AlignRight) != 0);
                if (str.length > f.width)
                {
                    sink(str[0 .. f.width]);
                    sink("...");
...
private void handle (T) (T v, FormatInfo f, scope FormatterSink sf, scope ElemSink se)
{
    ...
    // `v.toString()` and `T.IsSomething` pass when `typeof(v)` is an `Object*`,
    // and when `T` is a pointer to a type with `alias this`.
    ...
    else static if (is(typeof(v.toString(sf))))
    {
        ...
        scope sinkRef = (in char[] e) { se(e, f); };
        nullWrapper(&v,
                    v.toString(sinkRef),
                    se("null", f));
    }
    else static if (is(typeof(v.toString()) : const(char)[]))
    ...
    else static if (is(T A : A[]))
    {
        ...
        static assert (0, "Type unsupported by `dtext.format.Formatter`: "
                       ~ T.stringof);
}
...
private FormatInfo consume (scope FormatterSink sink, ref const(char)[] fmt)
{
    FormatInfo ret;
    auto s = fmt.ptr;
...
private const(char)[] forwardSlice (ref const(char)[] s, const(char)* p)
out (ret)
{
    assert(s.ptr == p);
...
do
{
    ...
    const old = s.ptr[0 .. cast(size_t) (p - s.ptr)];
    s = s[old.length .. $];
    return old;
}
